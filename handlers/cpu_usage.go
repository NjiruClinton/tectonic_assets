...
func GetCPUUsage(c echo.Context) error {
	...
	query := "SELECT process_name, usage, timestamp FROM cpu_usage WHERE 1=1"
	args := []interface{}{}
	argIndex := 1
...
func GetCPUProcesses(c echo.Context) error {
	query := "SELECT DISTINCT process_name FROM cpu_usage ORDER BY process_name"
